...
import { Card, CardBody, Col, Row, DropdownItem, DropdownMenu, DropdownToggle, UncontrolledDropdown } from 'reactstrap';
                            ...
                            <CardBody>
                                ...
                                <div className="d-flex align-items-end justify-content-between mt-4">
                                    ...
                                    <div className="avatar-sm flex-shrink-0">
                                        <span className={`${key === tileBoxs1.length - 3 ? "avatar-title bg-opacity-25 rounded fs-3 bg-" + item.bgiconClass : "avatar-title rounded fs-3 bg-" + item.bgiconClass + "-subtle"}`}>
                    ...
                    <Card className={"card-animate " + item.bgColor}>
                        <CardBody>
                            <div className="d-flex justify-content-between">
                                ...
                                <div>
                                    <div className="avatar-sm flex-shrink-0">
                                        <span className={`${key === tileBoxs3.length - 2 ? "avatar-title bg-opacity-25 rounded-circle fs-2 bg-" + item.bgiconClass : "avatar-title rounded-circle fs-2 bg-" + item.bgiconClass + "-subtle"}`}>
                                            <FeatherIcon
                                                icon={item.feaIcon}
                ...
                {(tileBoxes4 || []).map((item, key) => (<Col xl={3} md={6} key={key}>
                    <Card className={"card-height-100 " + item.bgColor}>
                        <CardBody>
                            <div className="d-flex align-items-center">
                                <div className="avatar-sm flex-shrink-0">
                                    <span className={`${key === tileBoxes4.length - 4 ? "avatar-title bg-opacity-25  rounded-2 fs-2 bg-" + item.bgiconClass + " text-" + item.iconClass : "avatar-title rounded-2 fs-2 bg-" + item.bgiconClass + "-subtle text-" + item.iconClass}`}>
                                ...
                                <div className="flex-shrink-0 align-self-center">
                                    <span className={`${key === tileBoxes4.length - 4 ? 'badge bg-' + item.percentageClass + ' bg-opacity-25 text-' + item.percentageClass + 'fs-12' : "fs-12 badge bg-" + item.percentageClass + "-subtle text-" + item.percentageClass}`}><i className={"fs-13 align-middle me-1 " + item.badge}></i>{item.percentage}<span>
                                    </span></span></div>
                            </div>
                        </CardBody>
                    </Card>
