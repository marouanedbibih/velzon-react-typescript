...
const ProjectsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(deleteProjectList.fulfilled, (state:any, action:any) => {
            // state.projectLists = state.projectLists.filter((project: any) => project.id.toString() !== action.payload.id);
            state.projectLists = state.projectLists.filter((project : any) => (project.id + "") !== (action.payload + ""));
        });
